#!/usr/bin/env python3
"""Contrastive pretraining with SetTransformer encoder and pooled view alignment."""
...
def collate_fn(batch):
    ...
    pooled = {}
    for key in pooled_views[0]:
        pooled[key] = torch.tensor(
            np.stack([pv[key] for pv in pooled_views]),
            dtype=torch.float32,
        )
...
def pretrain(args: argparse.Namespace) -> None:
    ...
    for epoch in trange(1, args.epochs + 1, desc="Contrastive pretraining (ST poolalign)"):
        encoder.train()
        epoch_loss = 0.0
        for batch in loader:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Contrastive pretraining with SetTransformer + pooled views.")
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "soap_pipeline_clean/outputs/set_transformer/contrastive_poolalign")
