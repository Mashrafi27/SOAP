...
class MOFSetTransformerTrainer:
    ...
    def _pad_soap_features(self, soap_out, soap, species):
        ...
        padded_features = np.zeros((n_atoms, len(self.all_soap_columns)))  # shape: [n_atoms, 484]

        for i, col in enumerate(current_columns):
            if col in self.all_soap_columns:
                col_idx = self.all_soap_columns.index(col)
                padded_features[:, col_idx] = soap_out[:, i]  # fill atom-wise values
    ...
    def _initialize_models(self):
        ...
        self.encoder_decoder = SetTransformerAggregation(**self.aggregator_params).to(device)
    ...
    def forward_pass(self, file_list, max_files=None):
        ...
        for file_path in files_to_process:
            try:
                ...
                soap_tensor = torch.FloatTensor(padded_soap).to(device)  # Shape: (1, 484)
                group_indices = torch.zeros(soap_tensor.size(0), dtype=torch.long).to(device)  # Single group
                aggregated = self.encoder_decoder(soap_tensor, index=group_indices)
                
                # aggregated shape: (1, aggregated_dim) 
                aggregated_features.append(aggregated.squeeze(0).to(device))
        ...
        features_tensor = torch.stack(aggregated_features).to(device)
