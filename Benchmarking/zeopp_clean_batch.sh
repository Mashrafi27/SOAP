...
echo "Processing $count CIF files..."
# CSV header
...
for cif in "$INPUT_DIR"/*.cif; do
  ...
  # write csv line
  echo "$base,$Di,$Df,$Dif,$density,$unitvol,$SA_A2,$SA_m2_cm3,$SA_m2_g,$AV_A3,$AV_frac,$AV_cm3_g,$numchan,$numpock" >> "$OUTPUT_CSV"
  ...
  if (( idx % 10 == 0 )); then
    ...
    echo "  $idx/$count done ($((elapsed/60))m$((elapsed%60))s)"
  fi
done
...
dur=$((end-start))
echo "Finished $idx files in $((dur/60))m$((dur%60))s"